...
export default function GoalsScreen() {
  ...
  const [items, setItems] = useState<GoalListRow[]>([]);

  useEffect(() => {
    if (!ready) return;
    void (async () => {
      const db = await getLocalDatabase();
      setItems(await listGoalsDecrypted(db));
    })();
  }, [ready, refreshKey]);
      ...
      {items.length === 0 ? (
        ...
        items.map((g) => (
          <View key={g.id} style={styles.card}>
            ...
            {g.motivation ? <Text style={styles.cardBody}>{g.motivation}</Text> : null}
            ...
            <View style={styles.switchRow}>
              ...
              <Switch
                ...
                onValueChange={(v) => {
                  void (async () => {
                    const db = await getLocalDatabase();
                    await setEntityLocalOnly(db, 'goal', g.id, v);
                    refresh();
                  })();
                }}
              />
