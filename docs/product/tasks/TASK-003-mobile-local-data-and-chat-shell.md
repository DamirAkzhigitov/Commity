...
## Acceptance Criteria
- [ ] App navigation includes exactly four tabs: Chat, Tasks, Reminders, and Settings.
- [ ] App includes mobile auth screens and secure session handling for signed-in assistant usage.
- [ ] Local SQLite tables exist for Tasks, Subitems, Documents, Reminders, chat messages, source metadata, privacy flags, and action history.
...
## Verification
- Manual Android flow: sign in, send chat, accept a proposed Task with Subitems, view it in the Tasks tab, undo it.

## Slice 2 (done): navigation shell + encrypted SQLite + proposals + privacy context
- **Tabs**: `(tabs)` layout is being narrowed to Chat, Tasks, Reminders, Settings (`app/(tabs)/`) to match MVP direction.
...
- **Next slice**: Task/Subitem/Document model, richer undo stacks, backend honoring `includeInAi`, confirmed-reminder scheduling (TASK-004).
